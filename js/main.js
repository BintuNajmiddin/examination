...
// let movieGenre = ['Action', 'Adventure', 'Animation', 'Comedy', 'Crime', 'Documentary', 'Drama', 'Family', 'Fantasy', 'History', 'Horror', 'Music', 'Mystery', 'Romance', 'Science Fiction', 'TV Movie', 'Thriller',  'War', 'Western'];

// function movie(id,poster,genres,lang,release_date, title){
//     this.id = id;
//     this.poster = poster;
//     this.genres = genres;
//     this.lang = lang;
...
const moviesDiv = document.getElementById('kino-id');
...
function createMovieDiv(movieObject){
    ...
    <h3 class="movie__genre">${movieObject.genres.join()}</h3>
...
const genreButtonsEl = document.getElementsByClassName('sidebar')[0];
if(genreButtonsEl){
    genreButtonsEl.addEventListener('click', function(event){
        if(event.target.className =='genre__item'){
            let selectedGenre = event.target.innerText;
            let selectedMovies;
            if(selectedGenre !== 'All'){
                selectedMovies = filterMoviesByGenre(selectedGenre);
            }else{
...
function appendSelectedMovies(smovies){
    smovies.forEach((el)=>{
        let kinoDiv = createMovieDiv(el);
        moviesDiv.appendChild(kinoDiv)
    })
}
...
if(moviesDiv){
    inputEl.addEventListener('keyup', function(){
        let searchedMovies = searchMovie(inputEl.value);
        moviesDiv.innerHTML = '';
...
if(moviesDiv){
    moviesDiv.addEventListener('click', function(event){
        ...
        addMovieToWatchList(idOfCurrentMovie);
    })
}

function createWatchItem(movieObject){
    let divEl = document.createElement('div');
    divEl.className = 'movie';
    divEl.innerHTML = `
    <img src="img/${movieObject.poster}" class="movie__poster">
    ...
    <h2 class="movie__title">${movieObject.title}</h2>
    <h3 class="movie__genre">${movieObject.genres.join()}</h3>
    <button class=" movie__btn remove" type="button" data-id="${movieObject.id}">Remove from watchlist</button>
    </div>`;
    return divEl;
}
const watchlistEl  = document.getElementById('films');
...
function showWatchList(){
    watchList.forEach((el)=>{
        let item = movies.filter((divEl)=>{
            return divEl.id  == el
        })
        watchlistEl.appendChild(createWatchItem(item[0]))
    })
